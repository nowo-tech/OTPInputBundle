...
2. **Product behavior** — what **OtpInputBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** form author, **I want** multi-box OTP inputs **so that** users enter one-time codes with a native mobile-friendly UI. |
| US-02 | **As an** integrator, **I want** a single Symfony form field value **so that** validation and submission stay standard. |
| US-03 | **As an** integrator, **I want** configurable length, separators, and autocomplete **so that** the widget matches my auth flow. |
| US-04 | **As a** maintainer, **I want** Vitest/PHPUnit in CI **so that** paste, backspace, and focus behavior do not regress. |
...
**Goal:** Symfony OTP FormType with multi-input UI mapped to a single field value.
